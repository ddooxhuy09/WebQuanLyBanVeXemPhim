...
<div class="modal" id="addModal" style="display: none;">
    <div class="modal-content">
        ...
        <form id="addForm" action="${pageContext.request.contextPath}/admin/food-combo/add" method="post" enctype="multipart/form-data">
            <div class="row">
                <div class="col-md-6">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="addMa" name="ma" value="${fn:escapeXml(newMaMap[addFormData != null ? addFormData.loai : 'Bắp Nước'])}" readonly>
                    </div>
                    <div class="form-group">
                        <label for="addTen">Tên</label>
                        <input type="text" class="form-control" id="addTen" name="ten" value="${fn:escapeXml(addFormData != null ? addFormData.ten : '')}" placeholder="VD: Bắp Rang Lớn" required>
                    </div>
                    <div class="form-group">
                        <label for="addGia">Giá (VNĐ)</label>
                        <input type="text" class="form-control" id="addGia" name="gia" value="${addFormData != null ? addFormData.gia : ''}" placeholder="VD: 50000" required>
                    </div>
                        ...
                        <div id="addBapNuocContainer">
                            <c:forEach var="bapNuoc" items="${allBapNuocList}">
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input combo-item-checkbox" id="addCb_${bapNuoc.maBapNuoc}" name="bapNuocIds" value="${bapNuoc.maBapNuoc}">
                                    ...
                                    <input type="number" class="form-control" name="soLuong_${bapNuoc.maBapNuoc}" value="${addFormData != null && addFormData.soLuongs != null ? addFormData.soLuongs[addFormData.bapNuocIds.indexOf(bapNuoc.maBapNuoc)] : '1'}" min="1" style="width: 60px; display: inline-block; margin-left: 10px;">
                                </div>
                            </c:forEach>
                        </div>
                    </div>
...
<div class="modal" id="editModal" style="display: none;">
    <div class="modal-content">
        <h3>Sửa Thông Tin</h3>
        <form id="editForm" action="${pageContext.request.contextPath}/admin/food-combo/edit" method="post" enctype="multipart/form-data">
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                    ...
                    <c:if test="${editLoai == 'Combo'}">
                        ...
                        <div class="form-group" id="editComboItemsField">
                            ...
                            <div id="editBapNuocContainer">
                                <c:forEach var="bapNuoc" items="${allBapNuocList}">
                                    <div class="form-check">
                                        <input type="checkbox" class="form-check-input combo-item-checkbox" id="editCb_${bapNuoc.maBapNuoc}" name="bapNuocIds" value="${bapNuoc.maBapNuoc}">
                                        ...
                                        <input type="number" class="form-control" name="soLuong_${bapNuoc.maBapNuoc}" value="<c:if test='${not empty editItem.chiTietCombos}'><c:forEach var='ct' items='${editItem.chiTietCombos}'><c:if test='${ct.maBapNuoc == bapNuoc.maBapNuoc}'>${ct.soLuong}</c:if></c:forEach></c:if><c:if test='${empty editItem.chiTietCombos}'>1</c:if>" min="1" style="width: 60px; display: inline-block; margin-left: 10px;">
                                    </div>
                                </c:forEach>
                            </div>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="image-view">
                            <c:if test="${not empty editItem.urlHinhAnh}">
                                <p>Tên file: ${fn:escapeXml(editItem.urlHinhAnh)}</p>
                                <img id="editHinhAnhPreview" src="${pageContext.request.contextPath}/${fn:escapeXml(editItem.urlHinhAnh)}" alt="Hình ảnh hiện tại" style="max-width: 100px; max-height: 100px;">
                            </c:if>
                ...
                <c:otherwise>
                    <c:forEach var="item" items="${bapNuocList}">
                        <tr data-loai="Bắp Nước">
                            ...
                            <td class="currency"><fmt:formatNumber value="${item.giaBapNuoc}" type="number" groupingUsed="true" minFractionDigits="0" maxFractionDigits="0"/>đ</td>
                            <td>Không có mô tả</td>
                            <td>
                                <c:if test="${not empty item.urlHinhAnh}">
                                    <img src="${pageContext.request.contextPath}/${item.urlHinhAnh}" alt="Hình ảnh" style="max-width: 50px; max-height: 50px;" onerror="this.src='${pageContext.request.contextPath}/resources/images/default-poster.jpg';"/>
                                </c:if>
                            ...
                            <td>Bắp Nước</td>
                            <td>
                                <a href="${pageContext.request.contextPath}/admin/food-combo?editMa=${item.maBapNuoc}&editLoai=Bắp Nước" class="custom-btn btn-sm mr-1">Sửa</a>
                                <form action="${pageContext.request.contextPath}/admin/food-combo/delete" method="post" style="display: inline;">
                                    <input type="hidden" name="ma" value="${item.maBapNuoc}">
                                    <input type="hidden" name="loai" value="Bắp Nước">
                                    <button type="submit" class="custom-btn btn-sm" onclick="return confirm('Bạn có chắc muốn xóa mục này không?')">Xóa</button>
                                </form>
                            </td>
                        </tr>
                    </c:forEach>
                    <c:forEach var="item" items="${comboList}">
                        <tr data-loai="Combo">
                            <td>${item.maCombo}</td>
                            <td>${item.tenCombo}</td>
                            <td class="currency"><fmt:formatNumber value="${item.giaCombo}" type="number" groupingUsed="true" minFractionDigits="0" maxFractionDigits="0"/>đ</td>
                            <td>${item.moTa}</td>
                            <td>
                                <c:if test="${not empty item.urlHinhAnh}">
                                    <img src="${pageContext.request.contextPath}/${item.urlHinhAnh}" alt="Hình ảnh" style="max-width: 50px; max-height: 50px;" onerror="this.src='${pageContext.request.contextPath}/resources/images/default-poster.jpg';"/>
                            ...
                            <td>Combo</td>
                            <td>
                                <a href="${pageContext.request.contextPath}/admin/food-combo?editMa=${item.maCombo}&editLoai=Combo" class="custom-btn btn-sm mr-1">Sửa</a>
                                <form action="${pageContext.request.contextPath}/admin/food-combo/delete" method="post" style="display: inline;">
