						...
						<div class="image-view">
							<c:if test="${not empty editItem.urlHinhAnh}">
								...
								<img id="editHinhAnhPreview"
									src="${pageContext.request.contextPath}/${fn:escapeXml(editItem.urlHinhAnh)}"
				...
				<c:otherwise>
					<c:forEach var="item" items="${bapNuocList}">
						<tr data-loai="Bắp Nước">
							...
							<td><c:if test="${not empty item.urlHinhAnh}">
									<img
										src="${pageContext.request.contextPath}/${item.urlHinhAnh}"
										alt="Hình ảnh" style="max-width: 50px; max-height: 50px;"
										onerror="this.src='${pageContext.request.contextPath}/resources/images/default-poster.jpg';" />
								</c:if> <c:if test="${empty item.urlHinhAnh}">
                                    Chưa có hình ảnh
                                </c:if></td>
							<td>Bắp Nước</td>
							<td><a
					...
					<c:forEach var="item" items="${comboList}">
						<tr data-loai="Combo">
							...
							<td>Combo</td>
							<td><a
								href="${pageContext.request.contextPath}/admin/food-combo?editMa=${item.maCombo}&editLoai=Combo"
								class="custom-btn btn-sm mr-1">Sửa</a>
								<form
									action="${pageContext.request.contextPath}/admin/food-combo/delete"
									method="post" style="display: inline;">
