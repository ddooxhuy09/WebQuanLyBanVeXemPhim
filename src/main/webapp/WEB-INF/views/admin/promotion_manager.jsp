...
<div class="modal" id="addModal" style="display: none;">
    <div class="modal-content">
        ...
        <form id="addForm" action="${pageContext.request.contextPath}/admin/promotions/add" method="post" onsubmit="return validateAddForm(this)">
            <div class="row">
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="addMaCode" name="maCode" placeholder="VD: CODE003" value="${addFormData != null ? addFormData.maCode : ''}">
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="addMoTa" name="moTa" rows="3" placeholder="VD: Giảm giá 20% cho vé xem phim">${addFormData != null ? addFormData.moTa : ''}</textarea>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select class="form-control" id="addLoaiGiamGia" name="loaiGiamGia">
                            <option value="Phần trăm" ${addFormData != null && addFormData.loaiGiamGia == 'Phần trăm' ? 'selected' : ''}>Phần trăm</option>
                            <option value="Số tiền cố định" ${addFormData != null && addFormData.loaiGiamGia == 'Số tiền cố định' ? 'selected' : ''}>Số tiền cố định</option>
                        </select>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" id="addGiaTriGiam" name="giaTriGiam" step="0.01" min="0" placeholder="VD: 20.00" value="${addFormData != null ? addFormData.giaTriGiam : ''}">
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control" id="addNgayBatDau" name="ngayBatDau" <c:if test="${addFormData != null && addFormData.ngayBatDau != null}">value="<fmt:formatDate value='${addFormData.ngayBatDau}' pattern='yyyy-MM-dd'/>"</c:if>>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="addNgayKetThuc">Ngày Kết Thúc</label>
                        <input type="date" class="form-control" id="addNgayKetThuc" name="ngayKetThuc" <c:if test="${addFormData != null && addFormData.ngayKetThuc != null}">value="<fmt:formatDate value='${addFormData.ngayKetThuc}' pattern='yyyy-MM-dd'/>"</c:if>>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select class="form-control" id="addApDungCho" name="apDungCho">
                            <option value="Tất cả khách hàng" ${addFormData != null && addFormData.apDungCho == 'Tất cả khách hàng' ? 'selected' : ''}>Tất cả khách hàng</option>
                            <option value="Khách hàng VIP" ${addFormData != null && addFormData.apDungCho == 'Khách hàng VIP' ? 'selected' : ''}>Khách hàng VIP</option>
                            <option value="Khách hàng mới" ${addFormData != null && addFormData.apDungCho == 'Khách hàng mới' ? 'selected' : ''}>Khách hàng mới</option>
                        </select>
                    </div>
                </div>
            </div>
        ...
        <tbody id="promotionList">
            <c:choose>
                <c:when test="${empty khuyenMaiList}">
                    <tr class="no-data">
                        <td colspan="9" nowrap="true">Không có dữ liệu</td>
                    </tr>
                </c:when>
                <c:otherwise>
                    <c:forEach var="km" items="${khuyenMaiList}">
                        <tr data-promotion-id="${km.maKhuyenMai}">
                            ...
                            <td>
                                ...
                                <a href="${pageContext.request.contextPath}/admin/promotions/delete/${km.maKhuyenMai}" class="custom-btn btn-sm" onclick="return confirm('Bạn có chắc chắn muốn xóa khuyến mãi ${km.maCode} không?');">Xóa</a>
                            </td>
                        </tr>
                    </c:forEach>
                </c:otherwise>
...
<div class="modal" id="editModal" style="display: none;">
    <div class="modal-content">
        ...
        <form id="editForm" action="${pageContext.request.contextPath}/admin/promotions/update" method="post" onsubmit="return validateEditForm(this)">
            <div class="detail-field">
                <label for="editMaKM">Mã KM</label>
                <input type="editMaKM" id="form-control" name="maKhuyenMai" readonly>
            </div>
            <div class="detail-field">
            ...
            <div class="detail-field">
                ...
                <select id="editLoaiGiamGia" name="loaiGiamGia" class="form-control">
                    ...
                    <option value="Số lượng cố định">Số lượng cố định</option>
                </select>
            </div>
            <div class="detail-field">
                <label for="editGiaTriGiam Giá trị Giá"></label>
            ...
            <div class="detail-field">
                <label for="editNgayKetThuc Ngày"></label>
...
<div class="modal" id="descriptionModal" style="display: none;">
    <div class="modal-content">
        ...
        <div class="modal-actions">
            <button class="modal-action" onclick="closeDescriptionModal()">Đóng</button>
        </div>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        <c:if test="${not empty addFormData}">
            ...
            document.getElementById('addApDungCho').value = '${addFormData.apDungCho}';
        </c:if>
        ...
        <c:if test="${not empty editFormData}">
            ...
            document.getElementById('editApDungCho').value = '${editFormData.apDungCho}';
        </c:if>
    });
    ...
    function updatePromotionList() {
        ...
        if (noDataRow && rows.length > 0) {
            ...
            newRow.innerHTML = `<td colspan="9" nowrap="true">Không có dữ liệu</td>`;
        ...
        document.querySelectorAll('td[data-field="moTa"]').forEach(cell => {
            ...
            const viewMore = cell.querySelector('.view-amount');
    ...
    function showEditModal(button) {
        ...
        document.getElementById('editGiaTriGiam').value = row.cells[4].textContent;
        document.getElementById('editNgayBatDau').value = row.cells[5].textContent;
        document.getElementById('editNgayKetThuc').value = row.cells[6].textContent;
        document.getElementById('editApDungCho').value = row.cells[7].textContent;
        document.getElementById('editModal').style.display = 'flex';
    }
    ...
    function validateAddForm(form) {
        ...
        const apDungCho = form.querySelector('#addApDungCho').value;
        ...
        if (!apDungCho) {
            errors.push("Trường áp dụng để không được để trống!");
        }
    ...
    function validateEditForm(form) {
        ...
        const apDungCho = form.querySelector('#editApDungCho').value;
