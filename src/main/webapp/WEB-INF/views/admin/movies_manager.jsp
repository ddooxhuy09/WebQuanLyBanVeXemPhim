...
.url-trailer-view a:hover {
    text-decoration: underline;
}
.url-trailer-view p, .url-trailer-edit {
    margin: 0;
}
</style>
...
<div class="filter-section mb-3">
    <form id="filterForm" action="${pageContext.request.contextPath}/admin/movies" method="get" class="form-inline">
        <div class="form-group mr-3">
            <label for="sort" class="mr-2">Sắp xếp theo:</label>
            <select class="form-control" id="sort" name="sort">
                <option value="all" ${sort == 'all' ? 'selected' : ''}>Tất cả</option>
                <option value="ngayKhoiChieu_asc" ${sort == 'ngayKhoiChieu_asc' ? 'selected' : ''}>Ngày phát hành gần nhất</option>
                <option value="ngayKhoiChieu_desc" ${sort == 'ngayKhoiChieu_desc' ? 'selected' : ''}>Ngày phát hành xa nhất</option>
            </select>
        </div>
        <div class="form-group mr-3">
            <label for="filterTheLoai" class="mr-2">Thể Loại:</label>
            <select class="form-control" id="filterTheLoai" name="theLoai">
                <option value="all" ${theLoai == 'all' ? 'selected' : ''}>Tất cả</option>
                <c:forEach var="tl" items="${theLoaiList}">
                    <option value="${tl.tenTheLoai}" ${theLoai == tl.tenTheLoai ? 'selected' : ''}>${tl.tenTheLoai}</option>
                </c:forEach>
            </select>
        </div>
        <div class="form-group mr-3">
            <label for="filterDinhDang" class="mr-2">Định Dạng:</label>
            <select class="form-control" id="filterDinhDang" name="dinhDang">
                <option value="all" ${dinhDang == 'all' ? 'selected' : ''}>Tất cả</option>
                <c:forEach var="dd" items="${dinhDangList}">
                    <option value="${dd.tenDinhDang}" ${dinhDang == dd.tenDinhDang ? 'selected' : ''}>${dd.tenDinhDang}</option>
                </c:forEach>
            </select>
        </div>
        <div class="form-group mr-3">
            <label for="filterDoTuoi" class="mr-2">Độ Tuổi:</label>
            <select class="form-control" id="filterDoTuoi" name="doTuoi">
                <option value="all" ${doTuoi == 'all' ? 'selected' : ''}>Tất cả</option>
        ...
        <div class="form-group mr-3">
            <label for="filterQuocGia" class="mr-2">Quốc Gia:</label>
            <select class="form-control" id="filterQuocGia" name="quocGia">
                <option value="all" ${quocGia == 'all' ? 'selected' : ''}>Tất cả</option>
                <option value="Việt Nam" ${quocGia == 'Việt Nam' ? 'selected' : ''}>Việt Nam</option>
                <option value="Mỹ" ${quocGia == 'Mỹ' ? 'selected' : ''}>Mỹ</option>
                <option value="Hàn Quốc" ${quocGia == 'Hàn Quốc' ? 'selected' : ''}>Hàn Quốc</option>
                <option value="Nhật Bản" ${quocGia == 'Nhật Bản' ? 'selected' : ''}>Nhật Bản</option>
            </select>
        </div>
        ...
        <tbody id="movieList">
            <c:forEach var="phim" items="${phimList}">
                <tr data-movie-id="${phim.maPhim}" data-ten-phim="${phim.tenPhim}"
                    ...
                    <td data-field="ngayKhoiChieu"><fmt:formatDate value="${phim.ngayKhoiChieu}" pattern="yyyy-MM-dd" /></td>
                    <td data-field="thoiLuong">${phim.thoiLuong}</td>
                    <td data-field="giaVe" class="currency"><fmt:formatNumber value="${phim.giaVe.longValue()}" type="number" groupingUsed="true" minFractionDigits="0" maxFractionDigits="0" />đ</td>
                    <td>
                        <a href="${pageContext.request.contextPath}/admin/movies?viewMaPhim=${phim.maPhim}&sort=${sort}&theLoai=${theLoai}&dinhDang=${dinhDang}&doTuoi=${doTuoi}&quocGia=${quocGia}" class="custom-btn btn-sm">Xem Chi Tiết</a>
                    </td>
                </tr>
            </c:forEach>
        </tbody>
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="addTenPhim" name="tenPhim" value="${tenPhim}" placeholder="VD: Phim C">
                    </div>
                    <div class="form-group">
                        <label for="addNhaSanXuat">Nhà Sản Xuất</label>
                        <input type="text" class="form-control" id="addNhaSanXuat" name="nhaSanXuat" value="${nhaSanXuat}" placeholder="VD: Nhà SX C">
                    </div>
                    <div class="form-group">
                        <label for="addQuocGia">Quốc Gia</label>
                        <input type="text" class="form-control" id="addQuocGia" name="quocGia" value="${quocGia}" placeholder="VD: Hàn Quốc">
                    </div>
                    <div class="form-group">
                        ...
                        <input type="hidden" name="theLoai" id="addTheLoaiHidden" value="${theLoai}">
                    </div>
                    <div class="form-group">
                        ...
                        <input type="hidden" name="dinhDang" id="addDinhDangHidden" value="${dinhDang}">
                    </div>
                    <div class="form-group">
                        <label for="addDoTuoi">Độ Tuổi</label>
                        <input type="number" class="form-control" id="addDoTuoi" name="doTuoi" value="${doTuoi}" placeholder="VD: 18" min="0">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="addDaoDien">Đạo Diễn</label>
                        <input type="text" class="form-control" id="addDaoDien" name="daoDien" value="${daoDien}" placeholder="VD: Đạo diễn C">
                    </div>
                    <div class="form-group">
                        ...
                        <input type="hidden" name="dvChinh" id="addDvChinhHidden" value="${dvChinh}">
                    </div>
                    <div class="form-group">
                        <label for="addNgayKhoiChieu">Ngày Khởi Chiếu</label>
                        <input type="date" class="form-control" id="addNgayKhoiChieu" name="ngayKhoiChieu" value="${ngayKhoiChieu}">
                    </div>
                    <div class="form-group">
                        <label for="addThoiLuong">Thời Lượng (phút)</label>
                        <input type="number" class="form-control" id="addThoiLuong" name="thoiLuong" value="${thoiLuong}" placeholder="VD: 130" min="1">
                    </div>
                    <div class="form-group">
                        <label for="addPoster">Poster</label>
                        <input type="file" class="form-control" id="addPoster" name="poster" accept="image/jpeg,image/png" onchange="validateFile(this)">
                        <small class="form-text text-muted">Chọn file hình ảnh (jpg, png, tối đa 5MB).</small>
                    </div>
                    <div class="form-group">
                        <label for="addUrlTrailer">URL Trailer</label>
                        <input type="url" class="form-control" id="addUrlTrailer" name="urlTrailer" value="${urlTrailer}" placeholder="VD: http://example.com/trailer.mp4">
                    </div>
                    <div class="form-group">
                        <label for="addGiaVe">Giá Vé</label>
                        <input type="text" class="form-control" id="addGiaVe" name="giaVe" value="${giaVe}" placeholder="VD: 120000">
                    </div>
                    <div class="form-group">
                        <label for="addMoTa">Mô Tả</label>
                        <textarea class="form-control" id="addMoTa" name="moTa" rows="4">${moTa}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control field-value" id="tenPhim" name="tenPhim" value="${phimModel.tenPhim}" readonly>
                    </div>
                    <div class="form-group">
                        <label for="nhaSanXuat">Nhà Sản Xuất</label>
                        <input type="text" class="form-control field-value" id="nhaSanXuat" name="nhaSanXuat" value="${phimModel.nhaSanXuat}" readonly>
                    </div>
                    <div class="form-group">
                        <label for="quocGia">Quốc Gia</label>
                        <input type="text" class="form-control field-value" id="quocGia" name="quocGia" value="${phimModel.quocGia}" readonly>
                    </div>
                    <div class="form-group">
                        <label for="theLoai">Thể Loại</label>
                        <div class="tag-container field-value" id="theLoaiContainer" style="display: none;">
                        ...
                        <div class="tag-display" id="theLoaiDisplay">
                            <c:choose>
                    ...
                    <div class="form-group">
                        ...
                        <div class="tag-container field-value" id="dinhDangContainer" style="display: none;">
                        ...
                        <div class="tag-display" id="dinhDangDisplay">
                            <c:choose>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control field-value" id="doTuoi" name="doTuoi" value="${phimModel.doTuoi}" readonly>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group">
                        <label for="daoDien">Đạo Diễn</label>
                        <input type="text" class="form-control field-value" id="daoDien" name="daoDien" value="${phimModel.daoDien}" readonly>
                    </div>
                    <div class="form-group">
                        <label for="dvChinh">Diễn Viên Chính</label>
                        <div class="tag-container field-value" id="dvChinhContainer" style="display: none;">
                        ...
                        <div class="tag-display" id="dvChinhDisplay">
                            <c:choose>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control field-value" id="ngayKhoiChieu" name="ngayKhoiChieu"
                               value="<fmt:formatDate value='${phimModel.ngayKhoiChieu}' pattern='yyyy-MM-dd'/>" readonly>
                    </div>
                    <div class="form-group">
                        <label for="thoiLuong">Thời Lượng (phút)</label>
                        <input type="number" class="form-control field-value" id="thoiLuong" name="thoiLuong" value="${phimModel.thoiLuong}" readonly>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="poster-view">
                            <c:if test="${not empty phimModel.urlPoster}">
                                ...
                                <img src="${pageContext.request.contextPath}/resources/images/${phimModel.urlPoster}"
                                     alt="Poster" style="max-width: 100px; max-height: 100px;"
                        ...
                        <div class="poster-edit" style="display: none;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="url" class="form-control field-value url-trailer-edit" id="urlTrailer" name="urlTrailer" value="${phimModel.urlTrailer}" style="display: none;">
                    </div>
                    <div class="form-group">
                        <label for="giaVe">Giá Vé</label>
                        <input type="text" class="form-control field-value" id="giaVe" name="giaVe" 
                               value="<fmt:formatNumber value='${phimModel.giaVe.longValue()}' type='number' groupingUsed='true' minFractionDigits='0' maxFractionDigits='0'/>" readonly>
                    </div>
                    <div class="form-group">
                        <label for="moTa">Mô Tả</label>
                        <textarea class="form-control field-value" id="moTa" name="moTa" rows="4" readonly>${phimModel.moTa}</textarea>
                    </div>
                </div>
            </div>
            <div class="modal-actions">
    ...
    if (addModal) {
        ...
        ['addTagContainer', 'addDinhDangContainer', 'addActorContainer'].forEach(containerId => {
            ...
            if (container) {
                ...
                console.error(`showAddModal: Container (${containerId}) not found`);
            }
        });
        initTagHandlers();
    } else {
        console.error('showAddModal: addModal not found');
    }
}
...
function validateEditForm(form) {
    ...
    if (!doTuoi || parseInt(doTuoi) < 0) errors.push("Độ tuổi phải là số không âm.");
    if (!daoDien) errors.push("Đạo diễn không được để trống.");
    if (!ngayKhoiChieu) errors.push("Ngày khởi chiếu không được để trống.");
