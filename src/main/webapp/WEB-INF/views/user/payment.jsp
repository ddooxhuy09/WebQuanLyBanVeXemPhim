          ...
          <c:when test="${not empty sessionScope.loggedInUser}">
            <li>
              <a href="${pageContext.request.contextPath}/user/profile"
                >Xin chào, ${sessionScope.loggedInUser.tenKhachHang}</a
              >
            </li>
            <li>
              <a
      ...
      <div class="booking-container">
        <div class="booking-left">
          <div class="payment-section">
            <h5>Khuyến mãi</h5>
            <form
              action="${pageContext.request.contextPath}/booking/apply-promo-code"
              method="post"
              class="promo-form"
            >
              <input type="hidden" name="maPhim" value="${maPhim}" />
            ...
            <c:if test="${not empty khuyenMai}">
              <div class="discount-info">
                ...
                <p>
                  Loại giảm giá: ${khuyenMai.loaiGiamGia == 'Phần trăm' ? 'Giảm
                  ' : 'Giảm cố định '}
                  <strong
                    ><fmt:formatNumber
          ...
          <div class="payment-section">
            ...
            <form
              action="${pageContext.request.contextPath}/booking/confirm-payment"
              method="post"
              id="paymentForm"
            >
              ...
              >
                <img
                  src="${pageContext.request.contextPath}/resources/images/vnpay.png"
                  alt="VNPAY"
                />
                <label>VNPAY</label>
          ...
          <c:if test="${not empty phuThuList}">
            <div class="surcharge-info">
              ...
              <p>
                <strong
                  >Tổng phụ thu (cho ${fn:length(selectedSeats.split(','))}
                  vé):</strong
                >
                <fmt:formatNumber
                  value="${tongPhuThu * fn:length(selectedSeats.split(','))}"
                  type="currency"
                  currencySymbol="đ"
                  groupingUsed="true"
                />
        ...
        <div class="booking-right">
          <div class="movie-info-right">
            <img
              src="${pageContext.request.contextPath}/resources/images/${phim.urlPoster}"
              alt="${phim.tenPhim}"
              style="width: 200px; height: auto"
            />
            <h2>${phim.tenPhim}</h2>
            <p>
              <strong>Rạp:</strong> ${rapChieu.tenRapChieu} - Rạp
              ${rapChieu.maRapChieu}
            </p>
            <p>
              <strong>Suất:</strong>
              <fmt:formatDate
                value="${suatChieu.ngayGioChieu}"
                pattern="HH:mm"
              />
              -
              <fmt:formatDate
            ...
            <p>
              ...
              <span class="price">
                <fmt:formatNumber
                  value="${phim.giaVe != null ? phim.giaVe : 90000}"
                  pattern="#,###"
                />đ
              </span>
            </p>
            <div class="booking-summary">
              ...
              <div class="price-details">
                ...
                <c:if test="${not empty selectedBapNuocs}">
                  ...
                  <c:forEach var="bapNuoc" items="${selectedBapNuocs}">
                    <p>
                      ...
                      <fmt:formatNumber
                        value="${bapNuocPrices[bapNuoc.key]}"
                        type="currency"
                        currencySymbol="đ"
                        groupingUsed="true"
                        maxFractionDigits="0"
                ...
                <c:if test="${not empty phuThuList}">
                  <p>
                    <strong
                      >Phụ thu (${fn:length(selectedSeats.split(','))}
                      vé):</strong
                    >
                    <fmt:formatNumber
                      value="${tongPhuThu * fn:length(selectedSeats.split(','))}"
              ...
              <button type="submit" form="paymentForm" class="confirm-btn">
                Xác nhận thanh toán
              </button>
            </div>
          </div>
